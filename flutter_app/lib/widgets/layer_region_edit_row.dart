...
/// Compact region display row + Gizmo activator button for Video Layers.
/// Provides 2-way real-time synchronization between Video Canvas Gizmo and Inspector inputs.
/// Supports all 4 layers: inpaint, primarySub, secondarySub, watermark.
class LayerRegionEditRow extends ConsumerWidget {
  final String label;
  final List<double>? region;
  final FrameLayerType layerType;
  final VoidCallback? onReset;
  final ValueChanged<List<double>>? onRegionChanged;
  final String nullLabel;

  const LayerRegionEditRow({
    super.key,
    required this.label,
  ...
  void _handleCoordChange(int index, double pctVal) {
    ...
    if (index == 0 && updated[0] >= updated[2]) {
      updated[0] = double.parse((updated[2] - 0.02).clamp(0.0, 1.0).toStringAsFixed(3));
    } else if (index == 1 && updated[1] >= updated[3]) {
      updated[1] = double.parse((updated[3] - 0.02).clamp(0.0, 1.0).toStringAsFixed(3));
    } else if (index == 2 && updated[2] <= updated[0]) {
      updated[2] = double.parse((updated[0] + 0.02).clamp(0.0, 1.0).toStringAsFixed(3));
    } else if (index == 3 && updated[3] <= updated[1]) {
      updated[3] = double.parse((updated[1] + 0.02).clamp(0.0, 1.0).toStringAsFixed(3));
    }
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    if (isActive) {
                      ...
                      // Turn on gizmo and select this layer
